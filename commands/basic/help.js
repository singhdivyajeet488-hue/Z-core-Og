...
module.exports = {
    ...
    async renderHelpView(interaction, viewData, message = null) {
        ...
        activePages.forEach((page, idx) => {
            const CATEGORY_ICONS = {
                media: "🎬", basic: "📁", utility: "🛠️", moderation: "🛡️", core: "🤖",
                lavalink: "🎵", fun: "🎮", distube: "🎶", setups: "⚙️", audio: "🔊"
            };
            const icon = CATEGORY_ICONS[page.category.toLowerCase()] || "📁";
        ...
        selectMenu.addOptions(rawOptions);
        ...
        const btnRow = new ActionRowBuilder().addComponents(
            ...
            new ButtonBuilder()
                ...
                .setEmoji('➡️')
                .setStyle(ButtonStyle.Primary)
                .setDisabled(viewData.currentPage === activePages.length),
            new ButtonBuilder()
                .setCustomId('help_toggle_mode')
                .setLabel(`${viewData.currentMode === 'slash' ? 'Prefix' : 'Slash'} Mode`)
                .setEmoji('🔄')
                .setStyle(ButtonStyle.Secondary)
        );

        const payload = {
            components: [...displayComponents, menuRow, btnRow],
            flags: MessageFlags.IsComponentsV2
        };
        ...
        if (message) {
            ...
            this.setupMainCollector(interaction, reply, viewData);
        }
    },

    setupMainCollector(interaction, message, viewData) {
        const collector = message.createMessageComponentCollector({ 
            time: 600000,
            dispose: true 
        });
        ...
        collector.on('collect', async (i) => {
            ...
            if (i.customId === 'help_menu_select') {
                const val = i.values[0];
                if (val === 'home') {
                    viewData.currentPage = 0;
                } else if (val.startsWith('page_')) {
                    viewData.currentPage = parseInt(val.split('_')[1], 10);
                }
            } else if (i.customId === 'help_prev') {
                if (viewData.currentPage > 0) viewData.currentPage--;
            } else if (i.customId === 'help_next') {
                const maxPages = viewData.chunkedPages[viewData.currentMode].length;
                if (viewData.currentPage < maxPages) viewData.currentPage++;
            } else if (i.customId === 'help_home') {
                viewData.currentPage = 0;
            } else if (i.customId === 'help_toggle_mode') {
                viewData.currentMode = viewData.currentMode === 'slash' ? 'prefix' : 'slash';
                viewData.currentPage = 0; 
            }

            await this.renderHelpView(i, viewData, message);
        });
